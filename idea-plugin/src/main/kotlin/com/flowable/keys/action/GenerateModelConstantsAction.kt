...
import com.flowable.keys.generate.ModelConstantsService
import com.flowable.keys.generate.ModelConstantsSettings
...
class GenerateModelConstantsAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        if (fqcn.endsWith(".") || fqcn.substringAfterLast('.').isBlank()) {
            Messages.showErrorDialog(project, "Invalid class name: '$fqcn'.", "Flowable Keys")
            return
        }

        val root = pickSourceRoot(project)
        if (root == null) {
            Messages.showErrorDialog(project, "No source root found to place the generated class.", "Flowable Keys")
            return
        }
