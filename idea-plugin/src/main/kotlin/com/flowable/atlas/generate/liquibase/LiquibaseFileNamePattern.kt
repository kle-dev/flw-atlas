...
object LiquibaseFileNamePattern {
    ...
    /** The service key with its trailing `[-_]?<letters><digits>` id segment removed (`DEMO-S009` → `DEMO`). */
    private fun servicePrefix(serviceKey: String): String =
        if (serviceKey.isBlank()) "" else serviceKey.replace(TRAILING_ID, "")

    /** The last digit run of the service key, leading zeros stripped (`DEMO-S009` → `9`; none → `""`). */
