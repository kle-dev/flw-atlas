 ...
 * when the environment is protected, and a red note when the stored pointer no longer resolves.
 *
 ...
 * A pointer that no longer resolves gets its own note rather than quietly reading "not set": the two need
 * different fixes, and silently swapping in another environment is the one thing this must never do.
 */
internal class EnvironmentPicker(private val project: Project, private val kind: ConnectionKind) {
    ...
    val combo = ComboBox<AtlasConnection?>().apply {
    ...
    private val note = JBLabel().apply { foreground = JBUI.CurrentTheme.Label.errorForeground() }
    ...
    fun fill(available: List<AtlasConnection>, resolution: Resolution, hasAnyEnvironment: Boolean) {
        ...
        placeholder = if (hasAnyEnvironment) message("hub.env.notSet") else message("hub.env.none")
        populating = true
        try {
        ...
        note.text = if (resolution is Resolution.Dangling) message("hub.env.removed") else ""
    }
    ...
    /** For tests: the row as a reader sees it — the name, or the placeholder, plus the note. */
    fun line(): String {
        val name = selected?.let { ConnectionLabels.pickerItem(it) } ?: placeholder
