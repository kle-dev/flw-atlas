...
class DiagramSvgCache {
    ...
    private data class Rendered(val stamp: Long, val picture: Picture, val file: VirtualFile)
    ...
    private fun rendered(modelFile: VirtualFile, type: ModelType): Rendered? {
        ...
        cached(modelFile.url)?.let { if (it.stamp == stamp) return it }
        ...
        val picture = runCatching { picture(bytes, modelFile.name, type) }
        ...
        val r = Rendered(stamp, picture, LightVirtualFile(name, picture.svg))
        remember(modelFile.url, r)
        return r
    }

    /** The picture of a model's [bytes] — the shared `:core` dispatch, see [ModelPicture]. */
    internal fun picture(bytes: ByteArray, fileName: String, type: ModelType): Picture? =
        ModelPicture.render(bytes, fileName, type)
