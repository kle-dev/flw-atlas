...
/**
 * Alt+Enter on a model key in Java → the model's page in the Atlas Explorer, inside the IDE.
 *
...
class OpenInAtlasExplorerIntention : IntentionAction, DumbAware {
    ...
    override fun isAvailable(project: Project, editor: Editor?, file: PsiFile?): Boolean =
        file is PsiJavaFile && editor != null && entryAt(project, file, editor.caretModel.offset) != null
    ...
    companion object {
         ...
         * The cached index only: an intention is asked on every Alt+Enter, and must never build one.
         */
        fun entryAt(project: Project, file: PsiFile, offset: Int): ModelEntry? {
            val leaf = file.findElementAt(offset) ?: return null
            val (key, types) = keyAt(leaf) ?: return null
