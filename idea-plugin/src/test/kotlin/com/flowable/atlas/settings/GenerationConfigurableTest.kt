...
class GenerationConfigurableTest : BasePlatformTestCase() {
    ...
    fun testThePageBuildsAndRoundTripsTheDtoClassNamePattern() {
        ...
        settings.dtoRenameFind = "^KYC"
        settings.dtoRenameReplace = "Kyc"
        ...
        try {
            ...
            settings.dtoClassNamePattern = "Kyc{name}Dto"
            configurable.reset()
            assertFalse(configurable.isModified)
            configurable.apply()
            assertEquals("Kyc{name}Dto", settings.dtoClassNamePattern)
            assertEquals("^KYC", settings.dtoRenameFind)
            assertEquals("Kyc", settings.dtoRenameReplace)
        } finally {
