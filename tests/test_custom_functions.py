...
def test_extracts_namespace_via_imported_binding(tmp_path):
    # The DEMO shape: entry re-exports an imported `additionalData` whose default export is the
    ...
    assert cat["namespaces"]["flowdemo"] == {"findCommon", "sortByDate"}
...
def test_extracts_from_compiled_rollup_bundle(tmp_path):
    ...
    _write(tmp_path, "src/main/resources/static/ext/custom.js",
           ...
           "  var additionalData = { flowdemo: { findCommon: findCommon, sortByDate: sortByDate },\n"
...
def test_extracts_from_minified_bundle_with_local_var_config(tmp_path):
    # Minified UMD (the real DEMO shape): the config is a local var referenced as `additionalData:a`,
    ...
    cat = fa.extract_custom_functions(str(tmp_path))
    assert cat is not None
    assert set(cat["namespaces"]["flowdemo"]) >= {"findCommonAttribute", "camelCaseToDashCase", "checkValidEmail"}
    assert cat["signatures"]["flowdemo.findCommonAttribute"] == "e, t, r"
    assert cat["signatures"]["flowdemo.camelCaseToDashCase"] == "e, t"
...
def test_recovers_real_param_names_from_sourcemap(tmp_path):
    ...
    cat = fa.extract_custom_functions(str(tmp_path))
    assert cat["signatures"]["flowdemo.findCommonAttribute"] == "allItems, path, identifierPath?"
    assert cat["signatures"]["flowdemo.camelCaseToDashCase"] == "str, toUpperCase"
...
_CAT = {"namespaces": {"flowdemo": {"findCommonAttribute", "sortByDateProperty"}},
        "flw": {"formatIban"}, "top_level": {"bareFn"}, "sources": [], "diagnostics": []}


def test_known_custom_namespace_member_is_valid():
    assert fa.validate_expression("{{ flowdemo.findCommonAttribute(x) }}", fa.FRONTEND, _CAT) == []


def test_typo_in_custom_namespace_member_is_suspect_with_suggestion():
    w = [p for p in fa.validate_expression("{{ flowdemo.findComonAttribute(x) }}", fa.FRONTEND, _CAT)
         if p["severity"] == "warning"]
    assert len(w) == 1
    assert w[0]["quickFix"] == "findCommonAttribute"
    assert w[0]["subject"] == "flowdemo.findComonAttribute"
...
def test_unknown_custom_member_without_near_match_stays_lenient():
    ...
    assert fa.validate_expression("{{ flowdemo.bananaSplitXyz(x) }}", fa.FRONTEND, _CAT) == []
...
def test_custom_namespace_member_findings_are_allowlistable():
    p = {"severity": "warning", "kind": "unknown-function", "subject": "flowdemo.findComonAttribute"}
    assert fa.expr_problem_allowlisted(p, {"flowdemo"})
    assert fa.expr_problem_allowlisted(p, {"flowdemo.findComonAttribute"})
...
def test_extract_auto_discovers_and_validates(tmp_path):
    ...
    _write(tmp_path, "fe/additionaldata/index.ts",
           'export default { flowdemo: { findCommonAttribute: () => 1 } };\n')
    # A form that calls a valid custom fn and one with a typo.
    _write(tmp_path, "models/ok.form",
           '{"metadata":{"key":"okForm","modelType":"form"},'
           '"rows":[[{"id":"a","type":"text","value":"{{ flowdemo.findCommonAttribute(x) }}"}]]}')
    _write(tmp_path, "models/typo.form",
           '{"metadata":{"key":"typoForm","modelType":"form"},'
           '"rows":[[{"id":"a","type":"text","value":"{{ flowdemo.findComonAttribute(x) }}"}]]}')
    r = fa.extract(str(tmp_path))
    cf = r["customFunctions"]
    assert cf and cf["namespaces"]["flowdemo"] == ["findCommonAttribute"]

    nodes = {n["id"]: n for n in r["graph"]["nodes"]}
    ok = nodes.get("binding:{{ flowdemo.findCommonAttribute(x) }}")
    typo = nodes.get("binding:{{ flowdemo.findComonAttribute(x) }}")
...
def test_custom_functions_become_cross_referenced_graph_nodes(tmp_path):
    ...
    _write(tmp_path, "models/reg.form",
           ...
           '"rows":[[{"id":"a","type":"text","value":"{{ flowdemo.findCommonAttribute(customer) }}"},'
           '{"id":"b","type":"text","value":"{{ flw.formatIban(iban) }}"}]]}')
    r = fa.extract(str(tmp_path))
    nodes = {n["id"]: n for n in r["graph"]["nodes"]}

    # a node per callable, including the unused top-level `greet`
    fc = nodes.get("customFunction:flowdemo.findCommonAttribute")
    assert fc is not None and fc["data"]["kind"] == "namespace" and fc["data"]["namespace"] == "flowdemo"
    ...
    assert "customFunction:flowdemo.findCommonAttribute" in used
    ...
    b = nodes.get("binding:{{ flowdemo.findCommonAttribute(customer) }}")
    assert b is not None and b["data"]["calls"] == ["customFunction:flowdemo.findCommonAttribute"]
...
def test_custom_function_signatures_are_extracted_and_shown(tmp_path):
    ...
    _write(tmp_path, "static/ext/custom.js",
           ...
           "}));\n")
    cat = fa.extract_custom_functions(str(tmp_path))
    assert cat["signatures"]["flowdemo.findCommon"] == "customer, docs"   # resolved from `function findCommon(…)`
    assert cat["signatures"]["flowdemo.calc"] == "a, b"                   # inline arrow
    assert cat["signatures"]["flw.formatIban"] == "iban"                # inline function expression
    r = fa.extract(str(tmp_path))
    nodes = {n["id"]: n for n in r["graph"]["nodes"]}
    assert nodes["customFunction:flowdemo.findCommon"]["label"] == "flowdemo.findCommon(customer, docs)"
...
def test_explorer_embeds_and_surfaces_custom_functions(tmp_path):
    ...
    _write(tmp_path, "fe/index.tsx",
           'import additionalData from "./additionaldata";\nexport default { additionalData };\n')
    _write(tmp_path, "fe/additionaldata/index.ts",
           'export default { flowdemo: { findCommonAttribute: () => 1, sortByDateProperty: () => 2 } };\n')
    r = fa.extract(str(tmp_path))
