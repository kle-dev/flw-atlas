...
class ModelParsersTest {
    ...
    fun parseFormExtractsOpUseFromDataSourceUrls() {
        ...
        val doc = """{"metadata": {"key": "DEMO-F045", "name": "F"},
                     "rows": [[{"id": "a", "type": "select", "label": "A", "extraSettings": {
                        "queryUrl": "{{endpoints.dataobject}}/dataobject-runtime/data-object-instances?dataObjectDefinitionKey=DEMO-DO2&dataObjectOperationKey=findActiveByIssueType&issueType={{issueType}}",
                        "lookupUrl": "{{endpoints.dataobject}}/dataobject-runtime/data-object-instances?dataObjectDefinitionKey=DEMO-DO2&dataObjectOperationKey=findActiveByIssueType"}}]]}""".toByteArray()
        val ctx = Ctx()
        ...
        assertTrue("DO op-use must be extracted from the data-source URL",
            Triple<Any?, Any?, Any?>("dataObject", "DEMO-DO2", "findActiveByIssueType") in opUses)
        assertTrue("form→dataObject ref must be recorded from the URL",
            Triple("field-dataObject", "dataObject", "DEMO-DO2") in refs(ctx))
    }
    ...
    fun parseFormUrlOpUseIgnoresDynamicKeys() {
        ...
        val doc = """{"metadata": {"key": "f1", "name": "F"},
                     "rows": [[{"id": "a", "type": "select", "label": "A", "extraSettings": {
                        "queryUrl": "{{endpoints.dataobject}}/x?dataObjectDefinitionKey=DEMO-DO2&dataObjectOperationKey={{op}}"}}]]}""".toByteArray()
        val ctx = Ctx()
        ModelParsers.parseForm(doc, ctx, "f.form")
