...
_Scanned 21 model files, 0 archives, 2 Java files, 2 REST endpoints. Generated by Flowable Atlas — regenerate with `atlas <project-dir>`. Diagrams of the models that carry layout: `miniproject.diagrams/*.svg`._
...
**Variables (30)** — where each one lives, is set and is read:
...
- `orderId` [process, query]
    - used in: openOrdersQuery {{orderId}}, orderProcess (declared / mapped) · passed as: in on `callSub`: orderId→subOrderId, in on `callSub`: orderId · touched by scripts: `Stamp order`, `Broken stamp`, `Broken API call`
- `orderNotes` [process]
...
**Expressions (27)** — grouped by what they call:
