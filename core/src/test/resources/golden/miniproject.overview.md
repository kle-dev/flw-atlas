...
_Scanned 27 models (27 files · 1 archives), 3 Java files, 2 REST endpoints. Generated by Flowable Atlas — regenerate with `atlas <project-dir>`. Diagrams of the models that carry layout: `miniproject.diagrams/*.svg`._
...
### Legacy order lookup (`DEMO-LF001`) — `form-models/DEMO-LF001.json`
- `panel1` [panel] Order
- `legacyOrderId` [text] Order id *(req)* ← `{{legacyOrderId}}`
- `orderSub` [subform] Order details
- `customerPick` [dataObjectSelect] Customer
- `loadOrder` [restButton] Load ← `{{$temp.loaded}}`
- `runCheck` [workAction] Run the check → action `notifyCustomerAction`
- `orderRows` [dataTable] Rows
- outcomes: `save`
- 🔌 data source: dataObject `customerDO.findAll`
- 🔌 data source: rest `/api/orders?customer={{customerPick}}`
- 📑 subform → `orderForm`
...
- `GET` /api/orders/{{legacyOrderId}}  _(from form-button `DEMO-LF001/loadOrder`)_
    - ⚠️ no matching controller in project (external or unimplemented)
- `GET` /api/orders?customer={{customerPick}}  _(from form-query `DEMO-LF001/orderRows`)_
    - ⚠️ no matching controller in project (external or unimplemented)
...
### Model → Java code
- `orderProcess` [serviceTask-expression] `demoBean` → src/main/java/com/example/DemoBean.java:6 (com.example.DemoBean)
...
- `contractDocument` —document-create-form→ form `DEMO-LF001` (`form-models/DEMO-LF001.json`)
...
- `DEMO-LF001` —outcome-form→ form `orderForm` (`forms/order-form.form`)
- `DEMO-LF001` —subform→ form `orderForm` (`forms/order-form.form`)
- `DEMO-LF001` —field-dataObject→ dataObject `customerDO` (`data/customer.data`)
- `DEMO-LF001` —triggers-action→ action `notifyCustomerAction` (`actions/notify-customer.action`)
...
- `legacyAction` [form]
    - used in: DEMO-LF001 (declared / mapped)
- `legacyMode` [form]
...
- `$route` (1)
    - `{{$route.currentAppId}}` — in `DEMO-LF001`
- `$row` (1)
    - `{{$row.id}}` — in `DEMO-LF001`
- `contactEmail` (1)
    - `{{contactEmail}}` — in `DEMO-LF001`
- `customerPick` (1)
    - `{{customerPick}}` — in `DEMO-LF001`
- `demoBean` (1)
...
- `legacyMode` (1)
    - `{{legacyMode == 'strict'}}` — in `DEMO-LF001`
- `legacyOrderId` (1)
    - `{{legacyOrderId}}` — in `DEMO-LF001`
- `manager` (1)
...
**literal secrets** (1)
- ⚠ `vectorStore.credentials.apiKey` holds a literal value — move it to an expression or an environment setting — in: `Courier manuals` · file: `knowledgebase/courier-manuals.knowledgebase`
