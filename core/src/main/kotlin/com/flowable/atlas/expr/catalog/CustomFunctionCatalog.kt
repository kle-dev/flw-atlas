 ...
 * top-level keys) and merges `additionalData.flw` onto `flw`. Projects
...
object CustomFunctionExtractor {
     ...
     *  shape where the config is a local var: `var …,a={flowdemo:{…}}` referenced by
    ...
    private fun absorbFromFile(path: File, cat: Cat, seen: MutableSet<String>, root: File): Boolean {
        ...
        if (!handled) {
            ...
            //      `var …,a={flowdemo:{…}}` … `return {…, additionalData:a}`).
            if (!handled) {
